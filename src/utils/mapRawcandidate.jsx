...
export const mapRawCandidate = (rawCandidate) => {
  return {
    ...
    // Vessel Experience (Mapping vessel types)
    vessel_experience: rawCandidate.experience?.vesselExperience?.map(vessel => ({
      vesselType: vessel.vesselType,
      experience: vessel.exp
    })),

    // Seagoing Experience (Mapping seagoing days and vessel names)
    seagoing_experience: rawCandidate.experience?.seagoingExperience?.map(sea => ({
      vesselName: sea.vesselName,
      netSeagoingDays: sea.netSeagoingDays
    }))
  };
};
